...
**Bindings match as a user-orderable list, first match wins.** The default order is that same specificity, so an untouched setup — and every existing rules test — behaves as before, while dragging in the dialog overrides it; the order rides in spare high bits of the existing registry value, so the format stays backward-compatible. A binding may **apply once** — set on the switch, then left alone (no enforcement poll), so a manual change sticks — and an opt-in **default** supplies a language only when nothing else matched.
...
Turning autostart off removes the Run entry and, for good measure, deletes any elevated logon task an older version created.

The Task Scheduler is reached through the COM API rather than by spawning `schtasks.exe`, and only ever to **find and delete** that legacy task — this version never creates one. Even for a delete, an unsigned process launching `schtasks.exe` is a signal worth not sending.

Both `[Run]` entries that start the utility after install use `runasoriginaluser`, so it launches at the user's normal integrity even though the admin installer is elevated. Without it the just-installed copy would inherit the installer's elevation, and its tray would hide **Restart as administrator** — not matching the unelevated state it has after a reboot, and leaving a player who follows the wiki unable to find the option. (These entries once used `shellexec`, needed while a since-removed `RUNASADMIN` compatibility layer made `CreateProcess` fail with 740; that layer forced every launch to elevate, which was the most malware-like thing the installer did, and antivirus was deleting the installer over it.)
